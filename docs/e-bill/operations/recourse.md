...
Recourse only becomes available once the bill is **only recoursable** — that is, after acceptance or
payment was refused or its deadline lapsed. At that point ordinary circulation is over and the
remaining question is who bears the loss.
...
2. **Iterate the holders until the caller finds themselves** as a holder or as the drawee. That
   locates the first block in which the caller held the bill — and they may only recourse against
   holders who held it *before* that point. Every holder encountered before it goes on the list.
...
they are not available — and Alice is both drawer and drawee, so step 4 does not add her and step 6
removes her.
...
the loss, they now hold the instrument and may pursue it themselves — including by recoursing
further back, against holders who preceded *their* first turn.
...
If the bill was recoursed to the **last possible recoursee** — nobody earlier remains — the bill is
**blocked permanently** and can only be resolved in court. This is the **recoursed to the end**
...
The **recoursee** — specifically the recoursee named in the last unexpired `RequestRecourse`, if that
is the last block.
...
fully verifiable, with no ability to write — which is one of the two use cases the sharing mechanism
